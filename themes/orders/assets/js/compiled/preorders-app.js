var preOrdersApp = (function () {
	...
	/* This file is generated by scripts/{"env":{"HUGO_ENV":"development"}}-messages/index.js. Do not edit! */
	...
	var root_2 = template(`<li><div class="columns is-align-items-center"><div class="column"><!> <div class="column"><h4 class="title has-text-info is-size-5"> </h4> <p class="is-size-6"> </p> <p class="is-size-5 my-3 has-text-info"> </p></div></div></div></li>`);
	var root_1 = template(`<ul></ul>`);
	var root = template(`<h1 class="title has-text-centered has-text-info"> </h1> <ul class="has-text-centered"><li> </li> <li> </li> <li> </li></ul> <div class="box my-6"><!></div>`, 1);
	...
	function PreOrder($$anchor, $$props) {
...

		{
			var consequent = ($$anchor) => {
				...
				each(ul_1, 5, () => order.products, index, ($$anchor, product) => {
					var li_3 = root_2();
					...
					template_effect(() => set_text(text_6, formatCurrency(get(product).price)));

					template_effect(() => {
						set_text(text_4, `${get(product).quantity ?? ""} x ${get(product).name ?? ""}`);
						set_text(text_5, get(product).description);
					});
			...
			if_block(node, ($$render) => {
				if (order.products.length > 0) $$render(consequent);
			});
		}

		template_effect(() => {
			set_text(text, `ORDER ${order.id}`);
			set_text(text_1, order.customer.name);
			set_text(text_2, order.customer.phone);
			set_text(text_3, order.customer.email);
		});

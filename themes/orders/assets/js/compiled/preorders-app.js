var preOrdersApp = (function () {
	...
	var root_5 = template(`<p class="is-size-5 my-3"> </p>`);
	...
	var root_6 = template(`<li><div class="column"><h4 class="title has-text-info is-size-4">Shipping Cost</h4> <p class="is-size-5 my-3"> </p></div></li>`);
	var root_2 = template(`<ul><!> <!> <li><div class="column"><hr> <h4 class="title has-text-info is-size-4 mt-5">Order total</h4> <p class="my-3"> </p></div></li></ul>`);
	...
	function PreOrder($$anchor, $$props) {
		...
		async function getOrder() {
			...
			const o = await fetch(`https://kettleblaze-store-server.fly.dev/order/${params.get("id")}`, { method: "GET" }).then((r) => r.json());
			...
			($$anchor, order) => {
					...
					var alternate = ($$anchor) => {
...

						{
							var consequent_5 = ($$anchor) => {
								...
								var text_16 = child(li_13, true);

								reset(li_13);

								var li_14 = sibling(li_13, 2);
								var text_17 = child(li_14, true);
              ...
              ${(get(order).customer.address.state ? `(${get(order).customer.address.state})` : "") ?? ""}`);
				...
				each(node_8, 1, () => get(order).events, index, ($$anchor, event) => {
...

					{
						var consequent_6 = ($$anchor) => {
							...
							const stringified_text = derived_safe_equal(() => new Date(get(event).ts).toLocaleDateString() ?? "");
							const stringified_text_1 = derived_safe_equal(() => new Date(get(event).ts).toLocaleTimeString() ?? "");
                  ...
                  ${get(stringified_text_1)}`));
							...
							var node_10 = sibling(li_18, 2);

							each(node_10, 1, () => get(event).data.parcels, index, ($$anchor, parcel, index) => {
								var li_19 = root_12();
						...
						var alternate_1 = ($$anchor) => {
							...
							const stringified_text_2 = derived_safe_equal(() => new Date(get(event).ts).toLocaleDateString() ?? "");
							const stringified_text_3 = derived_safe_equal(() => new Date(get(event).ts).toLocaleTimeString() ?? "");
                  ...
                  ${get(stringified_text_3)}`));
						...
						if_block(node_9, ($$render) => {
							if (get(event).type === "tracking-info") $$render(consequent_6); else $$render(alternate_1, false);
						});
					}
				...
				template_effect(() => {
					...
					set_text(text_9, `Phone: +${get(order).customer.address.country_data.phone[0] ?? ""}
